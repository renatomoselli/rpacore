...
class _UnexpectedFailStep(Step):
    def execute(self, ctx: ProcessContext) -> None:
...
def _build_transaction(item: QueueItem) -> Transaction:
    ...
    if mode == "success":
        step: Step = _SuccessStep("process_invoice", 1)
    elif mode == "skip":
        step = _SkipStep("process_invoice", 1)
    elif mode == "business":
        step = _BusinessFailStep("validate_invoice", 1)
    elif mode == "unexpected":
        step = _UnexpectedFailStep("submit_invoice", 1)
    else:
...
class TestEndToEndQueueWorkflow:
    def test_full_success_path(self, tmp_path) -> None:
        ...
        assert transaction.steps[0].status == Status.SUCCESSFUL
        ...
        assert report.steps[0].status == Status.SUCCESSFUL
    ...
    def test_explicit_skip_path_is_persisted_and_reported(self, tmp_path) -> None:
        ...
        transaction = transactions[0]
        assert transaction.status == Status.SUCCESSFUL
        assert transaction.steps[0].status == Status.SKIPPED

        assert callback_errors == [None]
        assert len(reports) == 1
        report = reports[0]
        assert report.status == Status.SUCCESSFUL
        assert report.steps[0].status == Status.SKIPPED
        assert report.steps[0].exceptions == []
    ...
    def test_business_exception_path(self, tmp_path) -> None:
        ...
        assert transaction.steps[0].status == Status.FAILED
        assert isinstance(transaction.steps[0].exceptions[0], BusinessException)
        ...
        assert report.steps[0].status == Status.FAILED
        assert isinstance(report.steps[0].exceptions[0], BusinessException)
    ...
    def test_unexpected_error_path_is_wrapped_and_reported(self, tmp_path) -> None:
        ...
        assert transaction.status == Status.FAILED
        assert transaction.steps[0].status == Status.FAILED
        assert isinstance(transaction.steps[0].exceptions[0], SystemException)
        assert str(transaction.steps[0].exceptions[0]) == "submit crashed"
        assert transaction.steps[0].exceptions[0].action == "submit_invoice"
        ...
        assert report.status == Status.FAILED
        assert report.steps[0].status == Status.FAILED
        assert isinstance(report.steps[0].exceptions[0], SystemException)
        assert str(report.steps[0].exceptions[0]) == "submit crashed"
