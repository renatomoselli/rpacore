"""Integration and idempotency tests for the greet_user example steps."""
...
class TestHappyPath:
    def test_all_steps_succeed(self, tmp_path):
        ...
        for step in tx.steps:
            assert step.status == Status.SUCCESSFUL
...
class TestBusinessExceptionPath:
    ...
    def test_execution_continues_after_business_exception(self, tmp_path):
        """Engine keeps running after BusinessException — downstream steps execute."""
        tx = _transaction(tmp_path, name=None)
        Engine().run(_ctx(tx))
    ...
    def test_transaction_fails_when_any_step_fails(self, tmp_path):
        tx = _transaction(tmp_path, name=None)
        Engine().run(_ctx(tx))
...
class TestSystemExceptionPath:
    ...
    def test_transaction_fails_when_confirm_output_fails(self, tmp_path):
        """Transaction where WriteGreeting is replaced by a step that does NOT
