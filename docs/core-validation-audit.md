...
| transaction state, metadata, skill arguments, and artifacts | Keep shared model validation on `Transaction`. | Execution, persistence, recovery, and serialization apply one JSON-safe durable-data contract without coercion. Reports and notifications retain defensive copies, stable timestamps, and content-free artifact records. These are durable model rules rather than config lookup rules. |
